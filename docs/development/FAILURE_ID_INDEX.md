...
As of the reconciliation on **2026-08-14**, identifiers through `F-029` are allocated. The next
identifier is `F-030` **only if a fresh repository-wide check confirms that no parallel branch has
...
| F-025 | Legacy JAXP provider rejected defense-in-depth XML access-control attributes | Implemented on ADAP branch; mandatory anti-XXE features retained. The next task-probe run still failed later in execution, so F-025 alone does not validate ADAP parity |
...
| F-027 | Push/PR CI events can enter the same concurrency group and leave no successful run | Proposal-ready governance defect; deterministic event-specific concurrency remains to be implemented/validated |
| F-028 | ADAP inventory referenced nonexistent `ModularADAPChromatogramBuilderParameters` | Proposal/source-provenance record on #50; canonical class is `ADAPChromatogramBuilderParameters`; no parity promotion |
...
The branch `agent/v408-chromatogram-feature-parity` is active and may move independently of PR #51.
At the documentation check on 2026-08-14 it had progressed beyond F-025 and contained:
...
its failures are globally registered, regressions are green, and the result is merged into
`open-offline-main`.
