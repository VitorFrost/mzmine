...
This branch is **not integrated parity evidence yet**. At the latest documentation check it had
progressed beyond F-025; the F-025 commit passed the general Open Offline CI but the dedicated ADAP
differential still failed after source preparation, compilation, and governed-input verification.
A later branch commit adjusted worker heap and triggered a new probe run. Until the strict/governed
